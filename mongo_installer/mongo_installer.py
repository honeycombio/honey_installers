...
Bailing out.""")
    ...
    if semver.compare(version, "2.6.0") < 0:
        # 2.4
        click.echo("""You're running an old version of mongo ({}). The most important reasons
...
def _check_and_fix_mongo_logging_level(username, password, auth_db, version):
...
class MongoInstaller(HoneyInstaller):
    ...
    def hook(self):
        ...
        mongo_username, mongo_password, mongo_auth_db = _check_mongo_connection()

        if (mongo_username, mongo_password, mongo_auth_db) != (None, None, None):
            _check_and_fix_mongo_logging_level(mongo_username, mongo_password, mongo_auth_db, version)
